...
func main() {
	...
	go job.NewSpotJob(channel.CurrencyPairMAPE_USDT, 50, 0.0015, channel.SecondKey, channel.SecondSecret).Start(context.TODO())
	...
	go job.NewSpotJob(channel.CurrencyPairXRP_USDT, 50, 0.0015, channel.SecondKey, channel.SecondSecret).Start(context.TODO())
	select {}
}
