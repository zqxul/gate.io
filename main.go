...
func main() {
	// go job.New(channel.CurrencyPairXRP_USDT, decimal.NewFromFloat(50), decimal.NewFromFloat(0.015), channel.Key, channel.Secret).Start()
	...
	go job.New(channel.CurrencyPairLTC_USDT, decimal.NewFromFloat(30), decimal.NewFromFloat(0.005), channel.Key, channel.Secret).Start()
